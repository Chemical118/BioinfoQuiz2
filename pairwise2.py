    ...
    for row in range(1, lenA + 1):
        for col in range(1, lenB + 1):
            ...
            if not penalize_end_gaps[0] and row == lenA:
                row_open = score_matrix[row][col - 1] + gap_B_fn(col, 1)
            ...
            print([nogap_score, row_open, col_open], [col, row])
            if not align_globally:
                best_score = max(nogap_score, row_open, col_open)
            else:
